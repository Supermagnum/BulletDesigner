...
class BulletTaskPanel:
    ...
    def __init__(self, bullet_obj):
        ...
        if hasattr(self, "land_diameter_label") and hasattr(
            self, "land_riding_checkbox"
        ):
    ...
    def _mark_initial_load_complete(self):
        ...
        App.Console.PrintMessage(
            "=== TaskPanel: Initial load complete - updates enabled ===\n"
        )
    ...
    def _create_ui(self):
        ...
        # Nose tab
        nose_tab = self._create_nose_tab()
        tabs.addTab(nose_tab, "Nose")
    ...
    def _create_basic_tab(self):
        ...
        self.land_riding_checkbox.setToolTip(
            "Body rides on land diameter (6.5mm), bands expand to groove diameter (6.7mm)"
        )
    ...
    def _create_nose_tab(self):
        ...
        layout.addRow("HP Diameter:", self.hp_diameter_spin)
        ...
        layout.addRow("Tip Length:", self.tip_length_spin)
        ...
        layout.addRow("Tip Base Diameter:", self.tip_base_diameter_spin)
        ...
        layout.addRow("Tip Tip Diameter:", self.tip_tip_diameter_spin)

        material_db = get_material_database()
        self.tip_material_combo = QtWidgets.QComboBox()
        self.tip_material_combo.addItems(material_db.get_tip_material_names())
        default_tip_idx = self.tip_material_combo.findText("PEEK")
        if default_tip_idx >= 0:
            self.tip_material_combo.setCurrentIndex(default_tip_idx)
        layout.addRow("Tip Material:", self.tip_material_combo)
        ...
        layout.addRow("Tip Density:", self.tip_density_spin)

        self.tip_category_label = QtWidgets.QLabel("")
        layout.addRow("Material Category:", self.tip_category_label)

        self.nose_validation_label = QtWidgets.QLabel("")
        self.nose_validation_label.setStyleSheet("color: red;")
        self.nose_validation_label.setWordWrap(True)
        layout.addRow("Validation:", self.nose_validation_label)
    ...
    def _connect_signals(self):
        ...
        self.nose_type_combo.currentTextChanged.connect(self._on_parameter_changed)
        self.hp_diameter_spin.valueChanged.connect(self._on_parameter_changed)
        self.hp_depth_spin.valueChanged.connect(self._on_parameter_changed)
        self.cavity_shape_combo.currentTextChanged.connect(self._on_parameter_changed)
        self.tip_length_spin.valueChanged.connect(self._on_parameter_changed)
        self.tip_base_diameter_spin.valueChanged.connect(self._on_parameter_changed)
        self.tip_tip_diameter_spin.valueChanged.connect(self._on_parameter_changed)
        self.tip_material_combo.currentTextChanged.connect(
            self._on_tip_material_changed
        )
        self.tip_material_combo.currentTextChanged.connect(self._on_parameter_changed)
        self.tip_density_spin.valueChanged.connect(self._on_parameter_changed)
    ...
    def _load_values(self):
        ...
        try:
            ...
            if hasattr(self.bullet_obj, "DesignType"):
                ...
                App.Console.PrintMessage(
                    "=== _load_values() - Loading DesignType ===\n"
                )
                App.Console.PrintMessage(
                    f"  bullet_obj.DesignType = {obj_design_type}\n"
                )
                App.Console.PrintMessage(
                    f"  Combo box currentText before load: {self.design_type_combo.currentText()}\n"
                )
                ...
                if index >= 0:
                    ...
                    App.Console.PrintMessage(
                        f"  Set combo box to index {index} ({obj_design_type})\n"
                    )
                else:
                    App.Console.PrintWarning(
                        f"  WARNING: DesignType '{obj_design_type}' not found in combo box!\n"
                    )
                self.design_type_combo.blockSignals(False)
                App.Console.PrintMessage(
                    f"  Combo box currentText after load: {self.design_type_combo.currentText()}\n"
                )
            if hasattr(self.bullet_obj, "LandRiding"):
                land_riding_val = bool(self.bullet_obj.LandRiding)
                App.Console.PrintMessage(
                    "=== _load_values() - Loading LandRiding ===\n"
                )
                App.Console.PrintMessage(
                    f"  bullet_obj.LandRiding = {land_riding_val}\n"
                )
                App.Console.PrintMessage(
                    f"  Checkbox isChecked() before load: {self.land_riding_checkbox.isChecked()}\n"
                )
                ...
                App.Console.PrintMessage(
                    f"  Checkbox isChecked() after load: {self.land_riding_checkbox.isChecked()}\n"
                )
                ...
                self.land_diameter_spin.setEnabled(land_riding_val)
                App.Console.PrintMessage(
                    "  Updated UI visibility: land_diameter_label/spin "
                    f"visible={land_riding_val}, enabled={land_riding_val}\n"
                )
            ...
            if hasattr(self.bullet_obj, "LandDiameter"):
                ...
                if land_dia_val < 1.0 or land_dia_val > 10.0:
                    App.Console.PrintWarning(
                        "TaskPanel: LandDiameter is wrong "
                        f"({land_dia_val}), displaying 6.50 in UI\n"
                    )
                    land_dia_val = 6.50
            ...
            if hasattr(self.bullet_obj, "Length"):
                ...
                if length_val < 10.0 or length_val > 100.0:
                    App.Console.PrintWarning(
                        "TaskPanel: Length is wrong "
                        f"({length_val}), displaying calculated value in UI\n"
                    )
            ...
            if hasattr(self.bullet_obj, "Weight"):
                ...
                if weight_val < 10.0 or weight_val > 1000.0:
                    App.Console.PrintWarning(
                        f"TaskPanel: Weight is wrong ({weight_val}), "
                        "displaying 140.0 in UI\n"
                    )
                    weight_val = 140.0
            ...
            if hasattr(self.bullet_obj, "BoatTailLength"):
                self.boat_tail_length_spin.setValue(
                    to_float(self.bullet_obj.BoatTailLength)
                )
            if hasattr(self.bullet_obj, "BoatTailAngle"):
                self.boat_tail_angle_spin.setValue(
                    to_angle_deg(self.bullet_obj.BoatTailAngle)
                )

            # Nose
            if hasattr(self.bullet_obj, "NoseType"):
                nose_name = str(self.bullet_obj.NoseType)
                mapping = {
                    "solid": "Solid",
                    "hp": "Hollow Point",
                    "hp_tip": "Hollow Point + Tip",
                }
                ui_value = mapping.get(nose_name, "Solid")
                idx = self.nose_type_combo.findText(ui_value)
                if idx >= 0:
                    self.nose_type_combo.setCurrentIndex(idx)
            if hasattr(self.bullet_obj, "HPDiameter"):
                self.hp_diameter_spin.setValue(to_float(self.bullet_obj.HPDiameter))
            if hasattr(self.bullet_obj, "HPDepth"):
                self.hp_depth_spin.setValue(to_float(self.bullet_obj.HPDepth))
            if hasattr(self.bullet_obj, "CavityShape"):
                idx = self.cavity_shape_combo.findText(str(self.bullet_obj.CavityShape))
                if idx >= 0:
                    self.cavity_shape_combo.setCurrentIndex(idx)
            if hasattr(self.bullet_obj, "TipLength"):
                self.tip_length_spin.setValue(to_float(self.bullet_obj.TipLength))
            if hasattr(self.bullet_obj, "TipBaseDiameter"):
                self.tip_base_diameter_spin.setValue(
                    to_float(self.bullet_obj.TipBaseDiameter)
                )
            if hasattr(self.bullet_obj, "TipTipDiameter"):
                self.tip_tip_diameter_spin.setValue(
                    to_float(self.bullet_obj.TipTipDiameter)
                )
            if hasattr(self.bullet_obj, "TipMaterial"):
                idx = self.tip_material_combo.findText(str(self.bullet_obj.TipMaterial))
                if idx >= 0:
                    self.tip_material_combo.setCurrentIndex(idx)
            if hasattr(self.bullet_obj, "TipDensity"):
                self.tip_density_spin.setValue(float(self.bullet_obj.TipDensity))
    ...
    def _on_nose_type_changed(self, nose_type_text):
        """Show/hide nose controls based on selected nose type."""
        ...
        for control in (
            ...
            self.hp_depth_spin,
            self.cavity_shape_combo,
        ):
            control.setEnabled(hp_enabled)
            control.setVisible(hp_enabled)
        ...
        for control in (
            ...
            control.setEnabled(tip_enabled)
            control.setVisible(tip_enabled)

        self._validate_nose_inputs()

    def _on_tip_material_changed(self, material_name):
        """Update tip density and category indicator from selected tip material."""
        material_db = get_material_database()
        tip_data = material_db.get_tip_material(material_name)
        if not tip_data:
            return

        self.tip_density_spin.setValue(float(tip_data.get("density_gcm3", 1.32)))
        category = tip_data.get("category", "Functional")
        max_vel = float(tip_data.get("max_velocity_ms", 0))
        text = f"{category} (max {max_vel:.0f} m/s)"
        if category == "Functional":
            color = "#1b7f3a"
        elif category == "Prototype":
            color = "#b26a00"
        else:
            color = "#b00020"
        self.tip_category_label.setText(text)
        self.tip_category_label.setStyleSheet(f"color: {color}; font-weight: bold;")
        self._validate_nose_inputs()

    def _validate_nose_inputs(self):
        """Validate nose input constraints. Returns (is_valid, error_msg)."""
        # During UI construction the nose tab is created before material tab.
        # Defer nose validation until all dependent widgets exist.
        if not hasattr(self, "density_spin"):
            self.nose_validation_label.setText("")
            return (True, "")

        self._update_nose_tooltips()
        nose_type_text = self.nose_type_combo.currentText()
        if nose_type_text == "Solid":
            self.nose_validation_label.setText("")
            return (True, "")

        d_effective = self.diameter_spin.value()
        body_length = max(
            0.0, self.length_spin.value() - self.boat_tail_length_spin.value()
        )
        hp_d = self.hp_diameter_spin.value()
        hp_depth = self.hp_depth_spin.value()

        if hp_d >= d_effective and d_effective > 0:
            msg = "HP Diameter must be less than effective diameter."
            self.nose_validation_label.setText(msg)
            return (False, msg)
        if hp_depth >= body_length and body_length > 0:
            msg = "HP Depth must be less than body length."
            self.nose_validation_label.setText(msg)
            return (False, msg)

        # Practical cavity termination rule in ogive:
        # HP_depth_max = ogive_length * 0.60
        hp_depth_min, hp_depth_max = calculate_hp_depth_limits(
            diameter_mm=d_effective,
            ogive_caliber_ratio=self.ogive_ratio_spin.value(),
        )
        if hp_depth_max > 0 and hp_depth > hp_depth_max:
            msg = (
                f"ERROR: HP depth {hp_depth:.2f} mm exceeds practical ogive limit "
                f"{hp_depth_max:.2f} mm. Recommended range is "
                f"{hp_depth_min:.2f}-{hp_depth_max:.2f} mm."
            )
            self.nose_validation_label.setText(msg)
            return (False, msg)

        # Material/velocity-aware HP diameter validation.
        # Task panel has no muzzle velocity input, so uses baseline 900 m/s.
        meplat_mm = self.meplat_spin.value()
        density_gcm3 = self.density_spin.value()
        ...
        if hp_msg:
            self.nose_validation_label.setText(hp_msg)
            return (False, hp_msg)

        if nose_type_text == "Hollow Point + Tip":
            tip_base = self.tip_base_diameter_spin.value()
            tip_tip = self.tip_tip_diameter_spin.value()
            tip_length = self.tip_length_spin.value()
            if tip_base > hp_d:
                msg = "Tip Base Diameter must be less than or equal to HP Diameter."
                self.nose_validation_label.setText(msg)
                return (False, msg)
            if tip_tip >= tip_base and tip_base > 0:
                msg = "Tip Tip Diameter must be less than Tip Base Diameter."
                self.nose_validation_label.setText(msg)
                return (False, msg)
            tip_data = get_material_database().get_tip_material(
                self.tip_material_combo.currentText()
            )
            ...
            ogive_length = (self.ogive_ratio_spin.value() * d_effective) / 2.0
            tip_validation = validate_tip_design(
                meplat_mm=meplat_mm,
                hp_depth_mm=hp_depth,
                ogive_length_mm=ogive_length,
                tip_length_mm=tip_length,
                tip_base_diameter_mm=tip_base,
                tip_tip_diameter_mm=tip_tip,
                tip_material_name=self.tip_material_combo.currentText(),
            )
            if tip_validation["errors"]:
                msg = f"ERROR: {tip_validation['errors'][0]}"
                self.nose_validation_label.setStyleSheet("color: #b00020;")
                self.nose_validation_label.setText(msg)
                return (False, msg)
            if tip_validation["warnings"]:
                msg = f"WARNING: {tip_validation['warnings'][0]}"
                self.nose_validation_label.setStyleSheet("color: #b26a00;")
                self.nose_validation_label.setText(msg)
                return (True, msg)

        self.nose_validation_label.setStyleSheet("color: #1b7f3a;")
        self.nose_validation_label.setText("")
        return (True, "")

    def _update_nose_tooltips(self):
        """Update dynamic tooltips with calculated maximum allowed values."""
        required_attrs = (
            "diameter_spin",
            "meplat_spin",
            "density_spin",
            ...
            "ogive_ratio_spin",
            "length_spin",
            "boat_tail_length_spin",
            "tip_base_diameter_spin",
            "tip_tip_diameter_spin",
            "tip_length_spin",
        )
        ...
        hp_d = self.hp_diameter_spin.value()
        hp_depth = self.hp_depth_spin.value()
        ogive_ratio = self.ogive_ratio_spin.value()
        body_length = max(
            0.0, self.length_spin.value() - self.boat_tail_length_spin.value()
        )

        hp_depth_min, hp_depth_max = calculate_hp_depth_limits(d_effective, ogive_ratio)
        if body_length > 0:
            hp_depth_max = min(hp_depth_max, max(0.0, body_length - 0.001))

        min_wall = min_wall_thickness(density_gcm3, 900.0)
        hp_diam_max_meplat = max(0.0, meplat_mm - (2.0 * min_wall))
        hp_diam_max_effective = max(0.0, d_effective - 0.001)
        ...
        self.hp_diameter_spin.setToolTip(
            ...
            f"(meplat/wall and diameter limits, 900 m/s baseline)."
        )
        ...
        ogive_length_mm = (ogive_ratio * d_effective) / 2.0 if d_effective > 0 else 0.0
        tip_length_max = max(
    ...
    def _on_weight_changed(self):
        ...
        App.Console.PrintMessage(
            f"=== _on_weight_changed() - Weight changed to {self.weight_spin.value()} grains ===\n"
        )
        ...
        if self.preview_checkbox.isChecked():
            ...
            App.Console.PrintMessage(
                "  Live Preview disabled - changes will apply on OK\n"
            )
    ...
    def _on_parameter_changed(self):
        ...
        App.Console.PrintMessage(
            "=== _on_parameter_changed() - Parameter changed ===\n"
        )
    ...
    def _on_land_riding_changed(self, state):
        ...
        if not self.bullet_obj:
            App.Console.PrintMessage(
                "=== _on_land_riding_changed() - No bullet_obj, returning ===\n"
            )
            return
        ...
        if hasattr(self, "_initial_load") and self._initial_load:
            App.Console.PrintMessage(
                "=== _on_land_riding_changed() - Skipped (initial load) ===\n"
            )
            return
        ...
        old_land_riding = (
            self.bullet_obj.LandRiding
            if hasattr(self.bullet_obj, "LandRiding")
            else "None"
        )
        App.Console.PrintMessage("=== _on_land_riding_changed() ===\n")
        ...
        App.Console.PrintMessage(
            f"  Checkbox isChecked(): {self.land_riding_checkbox.isChecked()}\n"
        )
        ...
        App.Console.PrintMessage(
            "  Updated UI visibility: land_diameter_label/spin "
            f"visible={is_checked}, enabled={is_checked}\n"
        )
        ...
        if hasattr(self.bullet_obj, "LandRiding"):
            ...
            App.Console.PrintMessage(
                f"  Verified bullet_obj.LandRiding = {self.bullet_obj.LandRiding}\n"
            )
    ...
    def _on_design_type_changed(self, design_type):
        ...
        if not self.bullet_obj:
            App.Console.PrintMessage(
                "=== _on_design_type_changed() - No bullet_obj, returning ===\n"
            )
            return
        ...
        if hasattr(self, "_initial_load") and self._initial_load:
            App.Console.PrintMessage(
                "=== _on_design_type_changed() - Skipped (initial load) ===\n"
            )
            return

        # Get current value before change
        old_design_type = (
            self.bullet_obj.DesignType
            if hasattr(self.bullet_obj, "DesignType")
            else "None"
        )
        App.Console.PrintMessage("=== _on_design_type_changed() ===\n")
        App.Console.PrintMessage(f"  Old DesignType: {old_design_type}\n")
        App.Console.PrintMessage(f"  New DesignType: {design_type}\n")
        App.Console.PrintMessage(
            f"  Combo box currentText: {self.design_type_combo.currentText()}\n"
        )
        ...
        if hasattr(self.bullet_obj, "DesignType"):
            ...
            App.Console.PrintMessage(
                f"  Verified bullet_obj.DesignType = {self.bullet_obj.DesignType}\n"
            )
    ...
    def _update_calculated_values(self):
        ...
        try:
            ...
            weight_val = (
                get_float_value(self.bullet_obj.ActualWeight)
                if hasattr(self.bullet_obj, "ActualWeight")
                else 0.0
            )
            self.actual_weight_label.setText(f"{weight_val:.1f} grains")

            volume_val = (
                get_float_value(self.bullet_obj.Volume)
                if hasattr(self.bullet_obj, "Volume")
                else 0.0
            )
            self.volume_label.setText(f"{volume_val:.2f} mm³")

            bc_val = (
                get_float_value(self.bullet_obj.BC_G1)
                if hasattr(self.bullet_obj, "BC_G1")
                else 0.0
            )
            self.bc_label.setText(f"{bc_val:.3f}")

            sd_val = (
                get_float_value(self.bullet_obj.SectionalDensity)
                if hasattr(self.bullet_obj, "SectionalDensity")
                else 0.0
            )
            self.sd_label.setText(f"{sd_val:.3f}")

            twist_val = (
                str(self.bullet_obj.RecommendedTwist)
                if hasattr(self.bullet_obj, "RecommendedTwist")
                else "N/A"
            )
    ...
    def apply(self):
        ...
        App.Console.PrintMessage(
            "=== TaskPanel.apply() - Applying parameter changes ===\n"
        )

        try:
            is_valid, _ = self._validate_nose_inputs()
            if not is_valid:
                App.Console.PrintWarning(
                    "TaskPanel: Nose validation failed. Changes not applied.\n"
                )
                return
            ...
            if (
                abs(new_weight - old_weight) > 0.01
            ):  # Weight changed (use smaller threshold)
            ...
            if hasattr(self.bullet_obj, "NoseType"):
                ...
                self.bullet_obj.NoseType = nose_map.get(
                    self.nose_type_combo.currentText(), "solid"
                )
            if hasattr(self.bullet_obj, "HPDiameter"):
                self.bullet_obj.HPDiameter = self.hp_diameter_spin.value()
            if hasattr(self.bullet_obj, "HPDepth"):
                self.bullet_obj.HPDepth = self.hp_depth_spin.value()
            if hasattr(self.bullet_obj, "CavityShape"):
                self.bullet_obj.CavityShape = self.cavity_shape_combo.currentText()
            if hasattr(self.bullet_obj, "TipLength"):
                self.bullet_obj.TipLength = self.tip_length_spin.value()
            if hasattr(self.bullet_obj, "TipBaseDiameter"):
                self.bullet_obj.TipBaseDiameter = self.tip_base_diameter_spin.value()
            if hasattr(self.bullet_obj, "TipTipDiameter"):
                self.bullet_obj.TipTipDiameter = self.tip_tip_diameter_spin.value()
            if hasattr(self.bullet_obj, "TipMaterial"):
                self.bullet_obj.TipMaterial = self.tip_material_combo.currentText()
            if hasattr(self.bullet_obj, "TipDensity"):
                self.bullet_obj.TipDensity = self.tip_density_spin.value()
    ...
    def _recalculate_dimensions_from_weight(self, target_weight):
        ...
        try:
            ...
            diameter_mm = (
                float(self.bullet_obj.Diameter.getValueAs("mm").Value)
                if hasattr(self.bullet_obj.Diameter, "getValueAs")
                else float(self.bullet_obj.Diameter)
            )
            land_diameter_mm = (
                float(self.bullet_obj.LandDiameter.getValueAs("mm").Value)
                if hasattr(self.bullet_obj.LandDiameter, "getValueAs")
                else float(self.bullet_obj.LandDiameter)
            )
            density = (
                float(self.bullet_obj.Density)
                if hasattr(self.bullet_obj, "Density")
                else 8.70
            )
            num_bands = (
                int(self.bullet_obj.NumBands)
                if hasattr(self.bullet_obj, "NumBands")
                else 4
            )
            band_length_mm = (
                float(self.bullet_obj.BandLength.getValueAs("mm").Value)
                if hasattr(self.bullet_obj.BandLength, "getValueAs")
                else float(self.bullet_obj.BandLength)
            )
            band_spacing_mm = (
                float(self.bullet_obj.BandSpacing.getValueAs("mm").Value)
                if hasattr(self.bullet_obj.BandSpacing, "getValueAs")
                else float(self.bullet_obj.BandSpacing)
            )
            ogive_caliber_ratio = (
                float(self.bullet_obj.OgiveCaliberRatio)
                if hasattr(self.bullet_obj, "OgiveCaliberRatio")
                else 7.0
            )
            ogive_type = (
                self.bullet_obj.OgiveType
                if hasattr(self.bullet_obj, "OgiveType")
                else "Tangent"
            )
            boat_tail_angle_deg = (
                float(self.bullet_obj.BoatTailAngle.getValueAs("deg").Value)
                if hasattr(self.bullet_obj.BoatTailAngle, "getValueAs")
                else float(self.bullet_obj.BoatTailAngle)
            )
            meplat_diameter_mm = (
                float(self.bullet_obj.MeplatDiameter.getValueAs("mm").Value)
                if hasattr(self.bullet_obj.MeplatDiameter, "getValueAs")
                else float(self.bullet_obj.MeplatDiameter)
            )
            land_riding = (
                bool(self.bullet_obj.LandRiding)
                if hasattr(self.bullet_obj, "LandRiding")
                else True
            )
            ...
            if result and result.get("valid", False):
                ...
                self.bullet_obj.BoatTailLength = App.Units.Quantity(
                    f"{boat_tail_length_mm:.2f} mm"
                )
                ...
                App.Console.PrintMessage(
                    ...
                    f"Length={total_length_mm:.2f}mm, "
                    f"BoatTail={boat_tail_length_mm:.2f}mm\n"
                )
        except Exception as e:
