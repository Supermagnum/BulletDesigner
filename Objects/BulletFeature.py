...
class BulletFeature:
    ...
    def __init__(self, obj):
        ...
        obj.addProperty(
            "App::PropertyLength", "LandDiameter", "Basic", "Land riding diameter in mm"
        )

        obj.addProperty(
            "App::PropertyLength", "Length", "Basic", "Total bullet length in mm"
        )

        obj.addProperty(
            "App::PropertyFloat", "Weight", "Basic", "Target weight in grains"
        )
        ...
        obj.addProperty(
            "App::PropertyEnumeration", "OgiveType", "Ogive", "Type of ogive profile"
        ...
        # Nose configuration
        nose_types = ["solid", "hp", "hp_tip"]
        obj.addProperty(
            "App::PropertyEnumeration",
            "NoseType",
            "Ogive",
            "Nose type: solid, hollow point, or hollow point with tip",
        )
        obj.NoseType = nose_types
        obj.NoseType = "solid"

        cavity_shapes = ["Cylindrical", "Conical"]
        obj.addProperty(
            "App::PropertyEnumeration",
            "CavityShape",
            "Ogive",
            "Hollow point cavity shape",
        )
        obj.CavityShape = cavity_shapes
        obj.CavityShape = "Cylindrical"

        obj.addProperty(
            "App::PropertyLength",
            "HPDiameter",
            "Ogive",
            "Hollow point cavity diameter in mm",
        )
        obj.HPDiameter = 1.0

        obj.addProperty(
            "App::PropertyLength", "HPDepth", "Ogive", "Hollow point cavity depth in mm"
        )
        obj.HPDepth = 1.0

        obj.addProperty(
            "App::PropertyLength", "TipLength", "Ogive", "Polymer tip length in mm"
        )
        obj.TipLength = 0.0
        ...
        obj.addProperty(
            ...
            "TipBaseDiameter",
            "Ogive",
            "Polymer tip base diameter in mm",
        )
        obj.TipBaseDiameter = 0.0

        obj.addProperty(
            "App::PropertyLength",
            "TipTipDiameter",
            "Ogive",
            "Polymer tip tip diameter in mm",
        )
        obj.TipTipDiameter = 0.0

        tip_material_names = material_db.get_tip_material_names()
        obj.addProperty(
            "App::PropertyEnumeration", "TipMaterial", "Ogive", "Ballistic tip material"
        )
        obj.TipMaterial = tip_material_names
        if tip_material_names:
            obj.TipMaterial = (
                "PEEK" if "PEEK" in tip_material_names else tip_material_names[0]
            )

        obj.addProperty(
            "App::PropertyFloat", "TipDensity", "Ogive", "Tip density in g/cm3"
        )
        obj.TipDensity = 1.32

        # Driving Bands
        obj.addProperty(
            "App::PropertyInteger", "NumBands", "Bands", "Number of driving bands (0-6)"
        )
        obj.NumBands = 4

        obj.addProperty(
            "App::PropertyLength", "BandLength", "Bands", "Length of each band in mm"
        )

        obj.addProperty(
            "App::PropertyLength", "BandSpacing", "Bands", "Space between bands in mm"
        )
        ...
        obj.addProperty(
            "App::PropertyEnumeration", "BaseType", "Base", "Base type"
        ).BaseType = base_types
        obj.BaseType = "BoatTail"

        obj.addProperty(
            "App::PropertyLength", "BoatTailLength", "Base", "Boat tail length in mm"
        )

        obj.addProperty(
            "App::PropertyAngle", "BoatTailAngle", "Base", "Boat tail angle in degrees"
        )
        ...
        obj.addProperty(
            "App::PropertyEnumeration", "Material", "Material", "Bullet material"
        )
        ...
        obj.addProperty(
            "App::PropertyFloat", "Density", "Material", "Material density in g/cm³"
        )
    ...
    def execute(self, obj):
        ...
        App.Console.PrintMessage(
            "=== execute() called - Recalculating bullet geometry ===\n"
        )
        ...
        try:
            ...
            for body_obj in doc.Objects:
                if (
                    hasattr(body_obj, "TypeId")
                    and body_obj.TypeId == "PartDesign::Body"
                ):
            ...
            for body_obj in bodies_to_fix:
                try:
                    ...
                    if hasattr(body_obj, "BaseFeature") and body_obj.BaseFeature == obj:
                        ...
                        App.Console.PrintMessage(
                            f"  Removed bullet from Body '{body_obj.Label}' BaseFeature\n"
                        )

                    # Remove from Group
                    if hasattr(body_obj, "Group") and obj in body_obj.Group:
                        body_obj.Group = [
                            item for item in body_obj.Group if item != obj
                        ]
                        App.Console.PrintMessage(
                            f"  Removed bullet from Body '{body_obj.Label}' Group\n"
                        )
                    ...
                    App.Console.PrintWarning(
                        f"  Could not remove bullet from Body '{body_obj.Label}': {e_body}\n"
                    )
        ...
        try:
            ...
            if length_val < 5.0:  # Unrealistically short
                App.Console.PrintLog(
                    f"Skipping execute() - Length too small ({length_val}mm), likely being reset\n"
                )
                return
        except Exception:
            pass
        ...
        try:
            ...
            if length_mm <= 0:
                ...
                App.Console.PrintLog(
                    f"Skipping execute() - Length not set yet (value: {length_mm})\n"
                )
                return
        except Exception as e:
        ...
        try:
            ...
            land_diameter_mm = (
                to_mm(obj.LandDiameter) if hasattr(obj, "LandDiameter") else 6.50
            )
            length_mm = to_mm(obj.Length) if hasattr(obj, "Length") else 32.0
            num_bands = int(obj.NumBands) if hasattr(obj, "NumBands") else 4
            band_length_mm = (
                to_mm(obj.BandLength) if hasattr(obj, "BandLength") else 2.25
            )
            band_spacing_mm = (
                to_mm(obj.BandSpacing) if hasattr(obj, "BandSpacing") else 3.0
            )
            ogive_caliber_ratio = (
            ...
            meplat_diameter_mm = (
                to_mm(obj.MeplatDiameter) if hasattr(obj, "MeplatDiameter") else 1.5
            )
            nose_type = str(obj.NoseType) if hasattr(obj, "NoseType") else "solid"
            hp_diameter_mm = (
                to_mm(obj.HPDiameter) if hasattr(obj, "HPDiameter") else 0.0
            )
            hp_depth_mm = to_mm(obj.HPDepth) if hasattr(obj, "HPDepth") else 0.0
            cavity_shape = (
                str(obj.CavityShape).lower()
                if hasattr(obj, "CavityShape")
                else "cylindrical"
            )
            tip_length_mm = to_mm(obj.TipLength) if hasattr(obj, "TipLength") else 0.0
            tip_base_diameter_mm = (
                to_mm(obj.TipBaseDiameter) if hasattr(obj, "TipBaseDiameter") else 0.0
            )
            tip_tip_diameter_mm = (
                to_mm(obj.TipTipDiameter) if hasattr(obj, "TipTipDiameter") else 0.0
            )
            boat_tail_length_mm = (
                to_mm(obj.BoatTailLength) if hasattr(obj, "BoatTailLength") else 5.0
            )
                ...
                try:
                    if hasattr(obj.BoatTailAngle, "getValueAs"):
                        boat_tail_angle_deg = float(
                            obj.BoatTailAngle.getValueAs("deg").Value
                        )
            ...
            if meplat_diameter_mm >= land_diameter_mm:
                meplat_diameter_mm = max(
                    0.1, land_diameter_mm * 0.5
                )  # Ensure minimum 0.1mm
                if hasattr(obj, "MeplatDiameter"):
                    obj.MeplatDiameter = App.Units.Quantity(
                        f"{meplat_diameter_mm:.2f} mm"
                    )
            ...
            if boat_tail_length_mm > max_boat_tail:
                ...
                if hasattr(obj, "BoatTailLength"):
                    obj.BoatTailLength = App.Units.Quantity(
                        f"{boat_tail_length_mm:.2f} mm"
                    )
                auto_corrected.append("boat_tail")

            # Nose-driven meplat override
            if nose_type == "hp" and hp_diameter_mm > 0:
                meplat_diameter_mm = hp_diameter_mm
            elif nose_type == "hp_tip" and tip_tip_diameter_mm > 0:
                meplat_diameter_mm = tip_tip_diameter_mm

            # Debug: Log values being passed to validation
            App.Console.PrintMessage("=== execute() validation inputs ===\n")
            App.Console.PrintMessage(
                f"  length_mm={length_mm}, diameter_mm={diameter_mm}\n"
            )
            App.Console.PrintMessage(
                "  num_bands="
                f"{num_bands}, band_length_mm={band_length_mm}, "
                f"band_spacing_mm={band_spacing_mm}\n"
            )
            App.Console.PrintMessage(
                f"  ogive_caliber_ratio={ogive_caliber_ratio}, "
                f"boat_tail_length_mm={boat_tail_length_mm}\n"
            )
            ...
            if num_bands > 0:
                ...
                App.Console.PrintMessage(
                    "  Validation calc: "
                    f"band_space={total_band_space}mm, "
                    f"available={available_length}mm\n"
                )
                App.Console.PrintMessage(
                    f"    ogive_length={ogive_length}mm "
                    f"(ratio={ogive_caliber_ratio} * diameter={diameter_mm} / 2)\n"
                )
            ...
            if not is_valid and error_msg:
                ...
                if not skip_error:
                    ...
                    App.Console.PrintMessage(
                        f"  (Values: length={length_mm}, bands={num_bands}, "
                        f"band_len={band_length_mm}, spacing={band_spacing_mm})\n"
                    )
            ...
            def _as_solid(shape_obj):
                """Convert FreeCAD shape wrappers to a usable solid when possible."""
                if isinstance(shape_obj, Part.Solid):
                    return shape_obj

                if isinstance(shape_obj, Part.Shape):
                    if hasattr(shape_obj, "Solids") and len(shape_obj.Solids) > 0:
                        return shape_obj.Solids[0]
                    if hasattr(shape_obj, "Shells") and len(shape_obj.Shells) > 0:
                        try:
                            return Part.Solid(shape_obj.Shells[0])
                        except Exception:
                            pass
                    try:
                        made = Part.makeSolid(shape_obj)
                        if isinstance(made, Part.Solid):
                            return made
                        if isinstance(made, Part.Shape) and hasattr(made, "Solids") and len(made.Solids) > 0:
                            return made.Solids[0]
                    except Exception:
                        pass
            ...
            solid_candidate = _as_solid(bullet_solid)
            if solid_candidate is not None:
                bullet_solid = solid_candidate

            # Apply hollow-point cavity and optional tip geometry
            if nose_type in ("hp", "hp_tip") and hp_diameter_mm > 0 and hp_depth_mm > 0:
                hp_radius = hp_diameter_mm / 2.0
                cavity_start = max(0.0, length_mm - hp_depth_mm)
                if cavity_shape == "conical":
                    cavity = Part.makeCone(
                        0.0,
                        hp_radius,
                        hp_depth_mm,
                        App.Vector(0, 0, cavity_start),
                        App.Vector(0, 0, 1),
                    )
                else:
                ...
                bullet_solid = bullet_solid.cut(cavity)
                solid_candidate = _as_solid(bullet_solid)
                if solid_candidate is not None:
                    bullet_solid = solid_candidate

            if nose_type == "hp_tip" and tip_length_mm > 0 and tip_base_diameter_mm > 0:
                tip_solid = Part.makeCone(
                    max(0.0, tip_base_diameter_mm / 2.0),
                    max(0.0, tip_tip_diameter_mm / 2.0),
                    tip_length_mm,
                    App.Vector(0, 0, length_mm),
                    App.Vector(0, 0, 1),
                )
                bullet_solid = bullet_solid.fuse(tip_solid)
                solid_candidate = _as_solid(bullet_solid)
                if solid_candidate is not None:
                    bullet_solid = solid_candidate
            ...
            if not isinstance(bullet_solid, Part.Solid):
                App.Console.PrintError(
                    f"  ERROR: Created shape is not a Part.Solid: {type(bullet_solid)}\n"
                )
            ...
            if not bbox.isValid():
                App.Console.PrintError("  ERROR: Solid has invalid bounding box\n")
                raise ValueError("Bullet solid has invalid bounding box")

            App.Console.PrintMessage(
                f"  Solid created: Volume={bullet_solid.Volume:.2f} mm³, "
            ...
            if hasattr(obj, "Shape") and not obj.Shape.isNull():
                ...
                try:
                    ...
                    App.Console.PrintMessage(
                        f"  Shape has {len(faces)} faces (available for sketch attachment)\n"
                    )
                except Exception:
                    pass
            ...
            try:
                ...
                if not bbox_final.isValid():
                    App.Console.PrintError(
                        "  ERROR: Shape bounding box invalid after assignment\n"
                    )
                    raise ValueError("Shape bounding box is invalid after assignment")

                App.Console.PrintMessage(
                    f"  Shape BBox: X=[{bbox_final.XMin:.2f}, {bbox_final.XMax:.2f}], "
            ...
            App.Console.PrintMessage(
                "=== execute() complete - Geometry recalculated ===\n"
            )
    ...
    def onChanged(self, obj, prop):
        ...
        if prop == "TipMaterial":
            try:
                if hasattr(obj, "TipDensity") and hasattr(obj, "TipMaterial"):
                    material_db = get_material_database()
                    obj.TipDensity = material_db.get_tip_density(
                        str(obj.TipMaterial), 1.32
                    )
            except Exception:
                pass
        ...
        if prop in geometry_props:
            App.Console.PrintMessage(
                f"=== onChanged() - Property '{prop}' changed, triggering recompute ===\n"
            )
                ...
                if prop == "Length":
                    length_val = (
                        ...
                        if "to_mm" in locals()
                            ...
                            obj.Length.getValueAs("mm").Value
                            if hasattr(obj.Length, "getValueAs")
                            else float(obj.Length)
                        )
                    )
                    if length_val < 5.0:
                        App.Console.PrintLog(
                            f"Skipping recompute in onChanged - Length too small ({length_val}mm)\n"
                        )
                        return
            ...
            if hasattr(obj, "recompute"):
                App.Console.PrintMessage(
                    f"  Calling obj.recompute() for property '{prop}'\n"
                )
                obj.recompute()
            else:
                App.Console.PrintWarning("  Object has no recompute() method!\n")
    ...
    def _update_calculated_properties(self, obj):
        ...
        try:
            ...
            if volume_mm3 > 0:
                ...
                weight_grains = calculate_weight_from_volume(volume_mm3, density)
            ...
            band_spacing_mm = (
                to_mm(obj.BandSpacing) if hasattr(obj, "BandSpacing") else 0.75
            )
            weight_grains = (
            ...
            tip_density = float(obj.TipDensity) if hasattr(obj, "TipDensity") else 1.32
            density = float(obj.Density) if hasattr(obj, "Density") else 8.86

            nose_result = calculate_nose_configuration(
                nose_type=nose_type,
                original_mass_grains=weight_grains,
                length_mm=length_mm,
                d_effective_mm=diameter_mm,
                bullet_density_gcm3=density,
                hp_diameter_mm=hp_diameter_mm,
                hp_depth_mm=hp_depth_mm,
                cavity_shape=cavity_shape,
                tip_length_mm=tip_length_mm,
                tip_base_diameter_mm=tip_base_diameter_mm,
                tip_tip_diameter_mm=tip_tip_diameter_mm,
                tip_density_gcm3=tip_density,
            )

            # Calculate bearing surface
            bearing_surface = calculate_bearing_surface(
                diameter_mm, length_mm, num_bands, band_length_mm, band_spacing_mm
            )
...
def makeBulletFeature(name="Bullet"):
    ...
    App.Console.PrintMessage(
        "=== makeBulletFeature: Calculating optimal dimensions from weight ===\n"
    )
    try:
        ...
        if design_type == "VLD":
            ...
            App.Console.PrintMessage(
                "Using VLD defaults: longer ogive, smaller meplat, longer boat tail\n"
            )
            ...
            obj.BoatTailLength = (
                8.0  # Longer boat tail (will be calculated, but start higher)
            )
        ...
        App.Console.PrintMessage("Input Parameters:\n")
        design_type_str = (
            str(obj.DesignType) if hasattr(obj, "DesignType") else "Traditional"
        )
        land_riding_str = (
            "Yes" if (hasattr(obj, "LandRiding") and obj.LandRiding) else "No"
        )
        ...
        App.Console.PrintMessage(
            f"  Groove Diameter: {get_display_value(obj.Diameter):.2f} mm\n"
        )
        App.Console.PrintMessage(
            f"  Land Diameter: {get_display_value(obj.LandDiameter):.2f} mm\n"
        )
        App.Console.PrintMessage(f"  Material Density: {density:.2f} g/cm³\n")
        App.Console.PrintMessage(f"  Number of Bands: {int(obj.NumBands)}\n")
        App.Console.PrintMessage(
            f"  Band Length: {get_display_value(obj.BandLength):.2f} mm\n"
        )
        App.Console.PrintMessage(
            f"  Band Spacing: {get_display_value(obj.BandSpacing):.2f} mm\n"
        )
        App.Console.PrintMessage(
            f"  Ogive Caliber Ratio: {float(obj.OgiveCaliberRatio):.2f}\n"
        )
        App.Console.PrintMessage(f"  Ogive Type: {str(obj.OgiveType)}\n")
        App.Console.PrintMessage(
            f"  Boat Tail Angle: {get_angle_display_value(obj.BoatTailAngle):.2f} deg\n"
        )
        App.Console.PrintMessage(
            f"  Meplat Diameter: {get_display_value(obj.MeplatDiameter):.2f} mm\n"
        )
        ...
        App.Console.PrintMessage("\n=== CALCULATED RESULTS ===\n")
        App.Console.PrintMessage(
            f"Total Length: {calc_result['total_length_mm']:.2f} mm\n"
        )
        App.Console.PrintMessage(
            f"Boat Tail Length: {calc_result['boat_tail_length_mm']:.2f} mm\n"
        )
        App.Console.PrintMessage(
            f"Bearing Surface Length: {calc_result['bearing_surface_length_mm']:.2f} mm\n"
        )
        App.Console.PrintMessage(
            f"Ogive Length: {calc_result['ogive_length_mm']:.2f} mm\n"
        )
        App.Console.PrintMessage(
            f"Gap Length Needed: {calc_result['gap_length_needed_mm']:.2f} mm\n"
        )
        App.Console.PrintMessage(
            f"Gap Coverage: {calc_result['gap_coverage_mm']:.2f} mm\n"
        )
        App.Console.PrintMessage("\nBallistic Properties:\n")
        App.Console.PrintMessage(
            f"  Ballistic Coefficient (G1): {calc_result['ballistic_coefficient_g1']:.3f}\n"
        )
        App.Console.PrintMessage(
            f"  Sectional Density: {calc_result['sectional_density']:.3f}\n"
        )
        App.Console.PrintMessage(
            f"  Length/Diameter Ratio: {calc_result['length_diameter_ratio']:.2f}\n"
        )
        ...
        App.Console.PrintMessage("\nWeight Verification:\n")
        App.Console.PrintMessage(
            f"  Target Weight: {calc_result['target_weight_grains']:.2f} grains\n"
        )
        App.Console.PrintMessage(
            f"  Calculated Weight: {calc_result['calculated_weight_grains']:.2f} grains\n"
        )
        App.Console.PrintMessage(
            f"  Weight Error: {calc_result['weight_error_percent']:.2f}%\n"
        )
        ...
        if calc_result["is_valid"]:
            ...
            App.Console.PrintMessage(
                f"Set Length = {calc_result['total_length_mm']:.2f} mm\n"
            )
            App.Console.PrintMessage(
                f"Set BoatTailLength = {calc_result['boat_tail_length_mm']:.2f} mm\n"
            )
        else:
            App.Console.PrintWarning("\nWARNING: Calculation failed!\n")
            App.Console.PrintWarning(f"Reason: {calc_result['validation_message']}\n")
            App.Console.PrintWarning("Attempting to use calculated values anyway...\n")
            # Try to use calculated values even if validation failed
            if calc_result["total_length_mm"] > 0:
                obj.Length = calc_result["total_length_mm"]
                obj.BoatTailLength = calc_result["boat_tail_length_mm"]
                App.Console.PrintMessage(
                    f"Using calculated Length = {calc_result['total_length_mm']:.2f} mm\n"
                )
                App.Console.PrintMessage(
                    "Using calculated BoatTailLength = "
                    f"{calc_result['boat_tail_length_mm']:.2f} mm\n"
                )
            else:
                # Fallback to defaults only if calculation completely failed
                App.Console.PrintWarning(
                    "Calculation returned zero length, using safe defaults\n"
                )
        ...
        try:
            ...
            App.Console.PrintMessage(
                "=== VERIFICATION: "
                f"Length={length_val}mm, Diameter={diameter_val}mm, "
                f"LandDiameter={land_diameter_val}mm, NumBands={num_bands_val} ===\n"
            )
            App.Console.PrintMessage(
                f"  BandLength={band_length_val}mm, "
                f"BandSpacing={band_spacing_val}mm, "
                f"OgiveRatio={ogive_ratio_val}\n"
            )
            ...
            if length_val < 10.0 or length_val > 100.0:
                App.Console.PrintWarning(
                    f"Length is incorrect ({length_val}mm), fixing to 32.0mm\n"
                )
                obj.Length = 32.0
                length_val = get_mm_value(obj.Length)
            ...
            if land_diameter_val < 1.0 or land_diameter_val > 10.0:
                App.Console.PrintWarning(
                    f"LandDiameter is incorrect ({land_diameter_val}mm), fixing to 6.50mm\n"
                )
                obj.LandDiameter = 6.50
                land_diameter_val = get_mm_value(obj.LandDiameter)
                App.Console.PrintMessage(
                    f"  After fix: LandDiameter={land_diameter_val}mm\n"
                )

            # Check if OgiveCaliberRatio is wrong and fix it
            if ogive_ratio_val < 2.0 or ogive_ratio_val > 15.0:
                App.Console.PrintWarning(
                    f"OgiveCaliberRatio is incorrect ({ogive_ratio_val}), fixing to 7.0\n"
                )
                obj.OgiveCaliberRatio = 7.0
                ogive_ratio_val = 7.0
                App.Console.PrintMessage(
                    f"  After fix: OgiveCaliberRatio={ogive_ratio_val}\n"
                )
            ...
            if meplat_val < 0.5 or meplat_val > 5.0:
                App.Console.PrintWarning(
                    f"MeplatDiameter is incorrect ({meplat_val}mm), fixing to 1.5mm\n"
                )
                obj.MeplatDiameter = 1.5
                App.Console.PrintMessage("  After fix: MeplatDiameter=1.5mm\n")

            # Check if BandLength is wrong and fix it
            if band_length_val < 0.5 or band_length_val > 10.0:
                App.Console.PrintWarning(
                    f"BandLength is incorrect ({band_length_val}mm), fixing to 2.25mm\n"
                )
                obj.BandLength = 2.25
                band_length_val = get_mm_value(obj.BandLength)
                App.Console.PrintMessage(
                    f"  After fix: BandLength={band_length_val}mm\n"
                )

            # Check if BandSpacing is wrong and fix it
            if band_spacing_val < 0.5 or band_spacing_val > 10.0:
                App.Console.PrintWarning(
                    f"BandSpacing is incorrect ({band_spacing_val}mm), fixing to 3.0mm\n"
                )
                obj.BandSpacing = 3.0
                band_spacing_val = get_mm_value(obj.BandSpacing)
                App.Console.PrintMessage(
                    f"  After fix: BandSpacing={band_spacing_val}mm\n"
                )

            # Check if BoatTailLength is wrong and fix it
            if boat_tail_length_val < 0.5 or boat_tail_length_val > 15.0:
                App.Console.PrintWarning(
                    f"BoatTailLength is incorrect ({boat_tail_length_val}mm), fixing to 5.0mm\n"
                )
                obj.BoatTailLength = 5.0
                boat_tail_length_val = get_mm_value(obj.BoatTailLength)
                App.Console.PrintMessage(
                    f"  After fix: BoatTailLength={boat_tail_length_val}mm\n"
                )
            ...
            if num_bands_val > 0:
                ...
                App.Console.PrintMessage(
                    f"  Band space needed: {total_band_space}mm, Available: {available_length}mm\n"
                )

                if total_band_space > available_length:
                    App.Console.PrintError(
                        "ERROR: Bands don't fit! Adjusting parameters...\n"
                    )
                    # Auto-adjust: reduce band spacing or length
                    if available_length > 0:
                        # Try to fit by reducing spacing
                        max_spacing = (
                            available_length - (num_bands_val * band_length_val)
                        ) / max(1, num_bands_val - 1)
                        if max_spacing > 0:
                            obj.BandSpacing = max(0.5, max_spacing)
                            App.Console.PrintMessage(
                                f"  Adjusted BandSpacing to {obj.BandSpacing}mm\n"
                            )
        except Exception as e:
